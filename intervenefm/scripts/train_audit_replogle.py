"""
Replogle K562 audit: same pipeline, single-perturbation 0/1 split.

For each held-out test gene, audit predicts under learned/mean/zero/random/identity/pop_mean.
Same metrics as Norman audit.
"""
...
def parse_args():
    ...
    ap.add_argument("--out_tag", default="replogle")
...
def main():
    ...
    if len(split['test_genes']) > args.max_test_genes:
        ...
        # Move now-non-test cells back to train
        train_set = set(split['train_cells'])
        # this requires rebuild — easier: rebuild from scratch
        train_cells = []
        for ci, cell_id in enumerate(adata.obs.index):
            if n_pert_arr[ci] == 0:
        ...
        print(f"[run_replogle] capped test_genes={len(split['test_genes'])}, train={len(train_cells)}, test={len(new_test)}")
    ...
    print(f"[run_replogle] params {sum(p.numel() for p in model.parameters())/1e6:.2f}M")
    ...
    # Save trained checkpoint for diagnostic analysis (L2 norms etc.)
    ...
    # === Audit ===
    # Build training pert-gene indices
    ...
    print(f"[audit_replogle] train pert genes: {len(train_pert_gene_indices)}")
    ...
    print(f"[audit_replogle] train perturbed cells: {len(train_perturbed_cell_ids)}")

    # Manual audit per test gene (single perturbation)
    rng = np.random.default_rng(args.seed)
    ...
    for tg in split['test_genes']:
        # Find observed cells for this test gene
